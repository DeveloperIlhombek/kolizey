		...
		<footer className='container relative mx-auto grid grid-cols-1 gap-6 bg-slate-900 p-4 text-gray-50 shadow-inner shadow-[#c3b06eff] ring-0 backdrop-blur-sm transition duration-500 group-hover:duration-500  sm:p-6 md:grid-cols-9'>
			...
			<div className='col-span-1 grid grid-cols-1 gap-4 md:col-span-4 md:grid-cols-2'>
				<div className='flex flex-col gap-4 font-serif'>
					...
					<div className='relative z-10 flex flex-col gap-2'>
						{navlink.map(item => (
							<Link
								href={item.route}
								key={item.label}
								className='text-center text-sm font-semibold text-white md:text-left md:text-base'
							>
								{item.label}
							</Link>
						))}
					...
					<div className='before:absolute before:-z-10 before:h-24 top-3 before:w-24 before:rounded-full before:bg-yellow-200/70 before:blur-2xl before:transition before:duration-700  md:before:h-32 md:before:w-32  before:top-20 before:left-1 before:-translate-x-1/2  group-hover:before:-translate-y-8 group-hover:before:translate-x-8 md:before:left-24 md:before:top-12'></div>
				</div>
			</div>
			...
			<div className='col-span-1 flex flex-col items-center md:col-span-3 md:items-start'>
				...
				<iframe
					...
					title='Joylashuv'
				></iframe>
				<div className='mt-4 flex items-center justify-center gap-2 font-serif text-white md:justify-start'>
